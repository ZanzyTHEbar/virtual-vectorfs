...
// OpenEmbedProvider wraps GGUFProvider for embedding tasks with Qwen3-Embedding-0.6B defaults
type OpenEmbedProvider struct {
	*GGUFProvider
	matryoshkaDims int
}

// NewOpenEmbedProvider creates a new OpenEmbedProvider with Qwen3-Embedding-0.6B defaults
...
// EmbedText generates embeddings with Matryoshka support
...
// SetMatryoshkaDims sets the target dimension for Matryoshka embeddings
func (p *OpenEmbedProvider) SetMatryoshkaDims(dims int) {
	p.matryoshkaDims = dims
}

// GetMatryoshkaDims returns the current target dimension for Matryoshka embeddings
func (p *OpenEmbedProvider) GetMatryoshkaDims() int {
	return p.matryoshkaDims
}

// adjustToDims truncates or pads the embedding vector to the target dimensions
...
// OpenChatProvider wraps GGUFProvider for chat tasks with Qwen3-1.7B defaults
type OpenChatProvider struct {
	*GGUFProvider
	systemPrompt string
}

// NewOpenChatProvider creates a new OpenChatProvider with Qwen3-1.7B defaults
...
// GenerateText generates chat responses with system prompt
func (p *OpenChatProvider) GenerateText(ctx context.Context, userInput string, options ...llama.PredictOption) (string, error) {
	prompt := fmt.Sprintf("System: %s\n\nUser: %s\n\nAssistant:", p.systemPrompt, userInput)
	return p.GGUFProvider.GenerateText(ctx, prompt, options...)
}

// SetSystemPrompt sets the system prompt for chat interactions
func (p *OpenChatProvider) SetSystemPrompt(prompt string) {
	p.systemPrompt = prompt
}

// OpenVisionProvider wraps GGUFProvider for vision tasks with LLaMA 3.2 Vision 3B defaults
type OpenVisionProvider struct {
	*GGUFProvider
}

// NewOpenVisionProvider creates a new OpenVisionProvider with LLaMA 3.2 Vision 3B defaults
...
// GenerateText generates vision-aware responses (placeholder for multimodal)
func (p *OpenVisionProvider) GenerateText(ctx context.Context, prompt string, options ...llama.PredictOption) (string, error) {
	return p.GGUFProvider.GenerateText(ctx, prompt, options...)
}

// DescribeImage provides a placeholder for image description (future multimodal feature)
...
// GetSupportedImageFormats returns supported image formats (placeholder)
